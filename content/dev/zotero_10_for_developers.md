...
The `ItemTree` class was split into several classes ([`5ca1fbb167`](https://github.com/zotero/zotero/commit/5ca1fbb167)): `ItemTree` handles the virtualized table and columns, `ItemTreeRow` subclasses handle row data and rendering, and `CollectionViewItemTree` renders the items belonging to a collection tree selection.
...
`Zotero.CookieSandbox` no longer exists ([`cc76207a23`](https://github.com/zotero/zotero/commit/cc76207a23)). If your plugin used it to isolate cookies for HTTP requests, use `Zotero.HTTP.newCookieContext()`, which creates an isolated cookie jar backed by a Mozilla `userContextId`:
